...
Live credentials stay in Runtime-owned environment variables. API keys are not
Tool arguments, Plan fields, Evidence, or Trace attributes. Fixture mode uses
the exact same normalized evidence contract for deterministic CI.
"""
...
# Stable teaching fixtures; deliberately not represented as current observations.
FRED_FIXTURE = {
    "T5YIE": [
...
EIA_FIXTURE = {
    "PET.EMM_EPMR_PTE_NUS_DPG.W": [
        {"period": "2026-02-23", "value": "3.05"},
        {"period": "2026-03-02", "value": "3.12"},
        {"period": "2026-03-09", "value": "3.18"},
        {"period": "2026-03-16", "value": "3.22"},
    ],
}
...
class FREDAdapter:
    ...
    def fetch_series(self, series_id: str, label: str, unit: str, mode: str = "fixture") -> dict:
        _validate_inputs(series_id, label, unit, mode)
        if mode == "fixture":
            ...
            request_url = f"{FRED_BASE_URL}?{urlencode({'series_id': series_id, 'api_key': api_key, 'file_type': 'json', 'sort_order': 'asc'})}"
            payload = self._transport(request_url)
            rows = payload.get("observations") or []
            # Persist a public series page, never the credential-bearing request URL.
...
def _build_evidence(*, provider, series_id, label, unit, mode, history, publisher, source_uri) -> dict:
    if not history:
        raise ValueError(f"{provider} series {series_id} returned no observations")
    latest = history[-1]
    return {
        "kind": "evidence",
        ...
        "note": "Deterministic teaching fixture." if mode == "fixture" else "Live public-data observation.",
    }
...
def _normalize_fred_rows(rows: list[dict]) -> list[Point]:
    ...
    for row in rows:
        ...
        if value in {None, "", "."}:
            continue
        try:
            points.append(Point(period=str(row["date"]), value=float(value)))
...
def _normalize_eia_rows(rows: list[dict]) -> list[Point]:
    ...
    for row in rows:
        ...
        if not period or value in {None, "", "."}:
            continue
        try:
            points.append(Point(period=str(period), value=float(value)))
...
def _validate_inputs(series_id: str, label: str, unit: str, mode: str) -> None:
    if mode not in {"fixture", "live"}:
        raise ValueError("mode must be 'fixture' or 'live'")
    for name, value in (("series_id", series_id), ("label", label), ("unit", unit)):
