(function () {
  ...
  async function run() {
    ...
    byId('source-note').textContent = config.demo_scenario === 'live' ? '公开数据 · 无延时或故障注入' : config.demo_scenario === 'execution_race' ? '纸面成交事件 · 持久化后发送 · 重复 fill 去重' : config.demo_scenario === 'paper_fill_accounting' ? '纸面成交账本 · 报价与成交分离 · 幂等重试 · 结算 P&L' : config.demo_scenario === 'paper_portfolio_risk' ? '2s10s 纸面组合投影 · 净平行 DV01 限额 · 拦截不写入账本' : config.demo_scenario === 'model_live' ? '本地 OpenAI API · Key 不进入页面或 Trace · 模型输出仅为待校验提议' : '教学演示 · 公开历史快照 · 包含明确的延时/故障注入';
    update();
    try {
