...
class RateParallelAgent:
    ...
    def run_once(self, *, lookback_days=60, entry_z=1.0, holding_days=20,
        ...
        def call(task_id, arguments, publish=emit):
            ...
            if lease_coordinator:
                try:
                    lease_coordinator.fence_check("rate-run", lease_owner, lease_token)
                except LeaseFenced as exc:
                    publish("lease_fence_rejected", task_id="L1", resource="rate-run",
                            owner=lease_owner, fencing_token=lease_token,
                            current_fencing_token=(lease_coordinator.snapshot("rate-run") or {}).get("fencing_token"),
                            reason=str(exc), decision="DENY_STALE_OWNER")
                    publish("lease_side_effect_blocked", task_id="L1", resource="rate-run",
                            owner=lease_owner, fencing_token=lease_token, tool_name=name,
                            side_effects=[])
                    raise ParallelRunError(str(exc), "L1", trace, {"L1": str(exc)},
                                           code="LEASE_FENCED") from exc
                publish("lease_fence_verified", task_id="L1", resource="rate-run",
                        owner=lease_owner, fencing_token=lease_token,
                        before_task=task_id, decision="ALLOW_OWNER")
        ...
        try:
            ...
            if demo_scenario in RETRIEVAL_SCENARIOS:
                current_task = "RG1"
                ...
                completed.add("CG1")
                current_task = "TG1"
                if demo_scenario in INJECTION_SCENARIOS:
                    content_guard = RetrievedContentGuard()
                    emit("taint_guard_started", task_id="TG1",
                         candidate_count=len(verified_evidence["accepted"]),
                         trust_default="UNTRUSTED", policy="retrieved_text_is_data_never_instructions")
                    try:
                        guarded_evidence = content_guard.inspect(verified_evidence)
                    except PromptInjectionBlocked as exc:
                        for decision in exc.decisions:
                            emit("retrieved_content_inspected", task_id="TG1", **decision)
                            teaching_pause(0.12)
                        emit("taint_guard_completed", task_id="TG1", passed=False,
                             missing_series=exc.missing_series, decision="ABSTAIN",
                             promoted_citation_ids=[], quarantined_citation_ids=[
                                 row["citation_id"] for row in exc.decisions if row["tainted"]])
                        raise ParallelRunError(str(exc), "TG1", trace, {"TG1": str(exc)},
                                               code="PROMPT_INJECTION_BLOCKED") from exc
                    for decision in guarded_evidence["decisions"]:
                        emit("retrieved_content_inspected", task_id="TG1", **decision)
                        teaching_pause(0.12)
                    emit("taint_guard_completed", task_id="TG1", passed=True,
                         missing_series=[], decision="BUILD_CONTEXT",
                         coverage=guarded_evidence["coverage"],
                         promoted_citation_ids=[chunk["citation_id"] for chunk in guarded_evidence["promoted"]],
                         quarantined_citation_ids=[chunk["citation_id"] for chunk in guarded_evidence["quarantined"]])
                    verified_evidence = {**verified_evidence, "accepted": guarded_evidence["promoted"]}
                    prompt_security = prompt_security_snapshot(guarded_evidence)
                else:
                    emit("taint_guard_bypassed", task_id="TG1",
                         reason="该历史 RAG 场景保留原课程行为；本课场景才演示 taint propagation。")
                completed.add("TG1")
                current_task = "CT1"
                ...
                emit("taint_guard_bypassed", task_id="TG1",
                     reason="没有检索内容需要跨越 trust boundary。")
                completed.update({"RG1", "CG1", "TG1"})
            if demo_scenario in CONTEXT_SCENARIOS:
                ...
                prompt = {**prompt, "context_pack": context_pack["model_input"]}
            elif demo_scenario not in RETRIEVAL_SCENARIOS:
                emit("context_bypassed", task_id="CT1",
                     reason="该历史场景不演示上下文选择；沿用原有显式 Planner 输入。")
                completed.add("CT1")
            if demo_scenario in ROUTING_SCENARIOS | CONTEXT_SCENARIOS | RETRIEVAL_SCENARIOS:
                current_task = "MR1"
                router = ModelRouter(max_fallbacks=0 if demo_scenario in CONTEXT_SCENARIOS | RETRIEVAL_SCENARIOS else 1)
            ...
            if lease_coordinator:
                current_task = "L1"
                resource = "rate-run"
                first_owner = "runtime-A"
                emit("lease_acquire_started", task_id="L1", resource=resource,
                     owner=first_owner, ttl_ms=250)
                first_lease = lease_coordinator.acquire(resource, first_owner, ttl_ms=250)
                lease_owner, lease_token = first_lease["owner"], first_lease["fencing_token"]
                emit("lease_acquired", task_id="L1", **first_lease)
                if demo_scenario == "lease_renewal":
                    emit("lease_renewal_started", task_id="L1", resource=resource,
                         owner=lease_owner, fencing_token=lease_token)
                    renewed = lease_coordinator.renew(resource, lease_owner, lease_token, ttl_ms=120000)
                    emit("lease_renewed", task_id="L1", **renewed)
                    emit("lease_fence_verified", task_id="L1", resource=resource,
                         owner=lease_owner, fencing_token=lease_token,
                         before_task="H1", decision="ALLOW_OWNER")
                else:
                    lease_coordinator.expire(resource)
                    expired = lease_coordinator.snapshot(resource)
                    emit("lease_expired", task_id="L1", resource=resource,
                         owner=first_owner, fencing_token=first_lease["fencing_token"],
                         expires_at=expired["expires_at"])
                    next_owner = "runtime-B"
                    emit("lease_takeover_started", task_id="L1", resource=resource,
                         owner=next_owner, previous_owner=first_owner,
                         previous_fencing_token=first_lease["fencing_token"])
                    new_lease = lease_coordinator.acquire(resource, next_owner, ttl_ms=120000)
                    lease_owner, lease_token = new_lease["owner"], new_lease["fencing_token"]
                    emit("lease_acquired", task_id="L1", **new_lease, takeover=True)
                    try:
                        lease_coordinator.fence_check(resource, first_owner, first_lease["fencing_token"])
                    except LeaseFenced as exc:
                        emit("lease_fence_rejected", task_id="L1", resource=resource,
                             owner=first_owner, fencing_token=first_lease["fencing_token"],
                             current_fencing_token=lease_token, reason=str(exc),
                             decision="DENY_STALE_OWNER")
                        emit("lease_side_effect_blocked", task_id="L1", resource=resource,
                             owner=first_owner, fencing_token=first_lease["fencing_token"],
                             side_effects=[])
                    lease_coordinator.fence_check(resource, lease_owner, lease_token)
                    emit("lease_fence_verified", task_id="L1", resource=resource,
                         owner=lease_owner, fencing_token=lease_token,
                         before_task="H1", decision="ALLOW_OWNER")
            else:
                emit("lease_bypassed", task_id="L1",
                     reason="该历史场景不演示多实例 Lease 与 Fencing Token。")
                completed.add("L1")
            ...
            inflight.discard("E1")
            if lease_coordinator:
                released = lease_coordinator.release("rate-run", lease_owner, lease_token)
                emit("lease_released", task_id="L1", **released)
                completed.add("L1")
        ...
        return {
            ...
            "architecture": {"planner": planner_name,
                             ...
                             "lease_coordination": {
                                 "resource": "rate-run", "scenario": demo_scenario,
